"""
Run this script to create the xactimate_parser.py file
Usage: python create_parser.py
"""
...
class XactimateParser:
        ...
        while i < len(lines):
            ...
            if item_match:
                ...
                if i + 1 < len(lines):
                    ...
                    # Check for single-line format (all on one line)
    ...
    def _parse_multi_line_format(self, line_idx: int, line_num: str, description: str, lines: list, room: str) -> dict:
        ...
        # Find the line with "Quantity Unit"
        qty_unit_idx = -1
...
# Write the file
output_path = '../api/parsers/xactimate_parser.py'
...
print("Now run: python app.py")
                    ...
                    # Check for estimate 4 style (quantity + unit on one line, then individual values)
                    # Look ahead to see if line after next has just quantity and unit
                    check_line = next_line
                    if not re.match(r'[\\d,.]+\\s+[A-Z]{2,3}', next_line):
                        # Next line might be continuation of description, check line after
                        if i + 2 < len(lines):
                            check_line = lines[i + 2].strip()
                    
                    # If we find "quantity unit" pattern, it's multi-line format
                    if re.match(r'^[\\d,.]+\\s+[A-Z]{2,3}
...
print(f"✓ Successfully created {output_path}")
print("Now run: python app.py"), check_line):
                        try:
                    ...
                    elif re.search(single_pattern, next_line):
                        # Single line format
                        try:
...
print("Now run: python app.py")
        ...
        for idx, line in enumerate(data_lines[:5]):  # Check first 5 lines
            if re.match(qty_pattern, line):
                qty_unit_idx = idx
                break
        
        if qty_unit_idx >= 0:
            qty_unit_match = re.match(qty_pattern, data_lines[qty_unit_idx])
            quantity = self._parse_number(qty_unit_match.group(1))
            unit = qty_unit_match.group(2)
            
            # Add any lines before qty_unit as description continuation
            full_description = description
            for idx in range(qty_unit_idx):
                if data_lines[idx] and len(data_lines[idx]) > 2:
                    full_description += " " + data_lines[idx]
            
            # After quantity/unit, values are on separate lines
            remaining = data_lines[qty_unit_idx + 1:]
            
            # Estimate 4 format: Unit Price, RCV, (Deprec), ACV (only 4 values)
            # Estimate 1,2,5 format: Unit Price, Tax, O&P, RCV, <Deprec>, ACV (7 values)
            
            if len(remaining) >= 4 and len(remaining) < 6:
                # Estimate 4 style - fewer values
                unit_price = self._parse_currency(remaining[0])
                rcv = self._parse_currency(remaining[1])
                depreciation = self._parse_currency(remaining[2])
                acv = self._parse_currency(remaining[3])
                ...
                # Estimate 1,2,5 style - full values
                unit_price = self._parse_currency(remaining[0])
                tax = self._parse_currency(remaining[1])
                o_and_p = self._parse_currency(remaining[2])
                rcv = self._parse_currency(remaining[3])
                depreciation = self._parse_currency(remaining[4])
                acv = self._parse_currency(remaining[5])
                
                # Check for description continuation after financial data
                for idx in range(6, min(len(remaining), 10)):
                    next_desc = remaining[idx]
                    num_pattern = r'^[\\d,.<>]+
